...
![GitHub Workflow Status](https://github.com/tscritch/wavesurfer-record-plugin-wav/actions/workflows/typescript-library-starter.yml/badge.svg?branch=main)
...
1. `npx degit tscritch/wavesurfer-record-plugin-wav my-project` or click on the `Use this template` button on GitHub!
2. `cd my-project`
3. `npm install`
...
1. Set up the `NPM_TOKEN` secret in GitHub Actions ([Settings > Secrets > Actions](https://github.com/tscritch/wavesurfer-record-plugin-wav/settings/secrets/actions))
2. Give `GITHUB_TOKEN` write permissions for GitHub releases ([Settings > Actions > General](https://github.com/tscritch/wavesurfer-record-plugin-wav/settings/actions) > Workflow permissions)
